        ...
        <main class="max-w-2xl mx-auto pt-8 pb-24 sm:pt-16 sm:px-6 lg:max-w-7xl lg:px-8">
            <div class="px-4 space-y-2 sm:px-0 sm:flex sm:items-baseline sm:justify-between sm:space-y-0">
                <div class="flex sm:items-baseline sm:space-x-4">
                    <h1 class="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">Order
                        #{{$orderInfo->order_number}}</h1>
                </div>
                <p class="font-bold text-gray-600">Order Date
                ...
                <form action="{{ route('adminOrders.update', $orderInfo->id) }}"
                    ...
                    @else
                        <h2 class="text-red-500 font-weight-bold">Order Cancelled</h2>
                    @endif
                </form>
            </div>
                ...
                <div
                        ...
                        <div>
                            ...
                            <dd class="mt-3 mb-3 text-gray-500">
                                @if($orderInfo->shipping_company)
                                    <span class="block"><strong>Business name: </strong> {{$orderInfo->shipping_company}}</span>

                                @endif
                                <span class="block"><strong>Name: </strong> {{$orderInfo->shipping_name}} {{$orderInfo->shipping_surname}}</span>
                                ...
                                <span class="block"><strong>Phone number: </strong>{{$orderInfo->shipping_phone}}</span>
                            </dd>
                            <dt class="font-medium text-gray-900">Billing address</dt>
                            <dd class="mt-3 text-gray-500">
                                @if($orderInfo->billing_company)
                                    <span class="block"><strong>Business name: </strong> {{$orderInfo->billing_company}}</span>
                                    <span class="block"><strong>VAT NUMBER </strong> {{$orderInfo->billing_vat}}</span>
                                @endif
                                <span
                                ...
                                <span class="block"><strong>Phone number: </strong>{{$orderInfo->billing_phone}}</span>
                            </dd>
                            @if($orderInfo->notes)
                                <dd class="mt-4 text-gray-500">
                                    <span class="block"><strong>NoteS: </strong> {{$orderInfo->notes}}</span>
                                </dd>
                            @endif
                        </div>

                        <div>
                            <td class="font-medium text-gray-900">Payment Information</td>
                            @if($orderInfo->status == 'pending' || $orderInfo->status == 'processing' || $orderInfo->status == 'completed')
                                <div class="mt-3">
                                    @if($orderInfo->payment_method === 'paypal' && $orderInfo->is_paid === 1)
                                        <dd class="-ml-4 ">
                                            ...
                                            <div class="ml-3 mt-3">
                                                <h5 class="mb-2"><strong>Payment method:</strong></h5>
                    ...
                    <dl class="mt-8 divide-y divide-gray-200 text-sm lg:mt-0 lg:col-span-5">
                        ...
                        <div class="pt-4 flex items-center justify-between">
                            <dt class="font-medium text-gray-900">Total Order VAT INCLUDED</dt>
